...
def run_inference(image_path):
    ...
    for part, expected_count in expected_counts.items():
        defect_description = ""  # Initialize the defect_description to avoid unbound local variable error
        actual_count = object_counts[part]
        if actual_count < expected_count:
            defect_description = f"Missing {part}"
            defect_description = defect_description.replace("rotation", "")  # Remove the word "rotation"
            defects.append(defect_description)
            missing_elements.append(expected_count - actual_count)  # Track missing count
        elif actual_count > expected_count:
            defect_description = f"Too many {part}"
            defect_description = defect_description.replace("rotation", "")  # Remove the word "rotation"
            defects.append(defect_description)
            missing_elements.append(0)  # No missing element, so 0
    ...
    if not defects:
        ...
        # Create DataFrame with two columns: "Defect Description" and "Missing Count"
        df = pd.DataFrame({
            "Defect Description": defects,
            "Missing Count": missing_elements
        })
        print(df.head())
        df.to_csv('./df.csv', index=False)
        return 0
