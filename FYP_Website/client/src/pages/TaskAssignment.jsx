...
// import "../css/TaskAssignement.css";
...
const TaskAssignment = () => {
  ...
  return (
    <>
    {isAdmin == 1 && (     
      <h2 className="task-section-header" id="task-page" onClick={() => setIsExpanded(!isExpanded)}>
        Add New Task {isExpanded ? '-' : '+'}
      </h2>
    )}
    {isExpanded && (
      <div className="add-task-form">
        ...
        <div>
          <label className="add-task-label">Task Message:</label>
          <input
        ...
        <div>
          <label className="add-task-label">Task Duration:</label>
          <input
            className="add-task-input"
            type="text"
            value={taskDuration}
        ...
        <button className="add-task-btn" onClick={submitTask}>Add Task</button>
      </div>
    )}
    ...
    <div className="task-filter-container">
      {isAdmin == 1 && (
        <>
          <label className="task-filter-label">Select Employee:</label>
          <select 
            className="task-filter-select"
            value={selectedEmployeetable} 
            onChange={(e) => setSelectedEmployeetable(e.target.value)}
          >
            <option value="">All Employees</option>
            {[...new Set(tasksList.map(task => task.e_name))].map(name => (
              <option key={name} value={name}>{name}</option>
      ...
      <label className="task-filter-label">Completed:</label>
      <select 
        className="task-filter-select"
    ...
    <br />
    <TableContainer className="task-table-container" component={Paper}>
      <TextField
      ...
      <Table className="task-table">
        ...
        <TableBody className="task-table-body">
          {sortedAndFilteredTasks
            .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
            .map(task => (
              <TableRow key={task.task_assignment_id}>
